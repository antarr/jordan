...
RSpec.describe 'Password Change', type: :feature do
  ...
  describe 'Password Change Process', js: true do
    it 'successfully changes password with valid inputs' do
      ...
      expect(page).to have_current_path(%r{/profile/edit$}, wait: 10)

      # Use a more specific wait for the success message
      expect(page).to have_content('Password changed successfully', wait: 10)
    end
